// app/iptv-vs-[competitor]/page.tsx
...
const listStyle: React.CSSProperties = { paddingLeft: 22, lineHeight: 1.8 };

import { COMPETITORS_DATA, versusPageData } from '@/content/programmatic-pages';
...
export async function generateMetadata({ params }: { params: Promise<{ competitor: string }> }): Promise<Metadata> {
  ...
  const data = versusPageData(competitor);
  if (!data) return { title: 'Comparison not found' };
  return {
    title: data.metadata.title,
    description: data.metadata.description,
    keywords: data.metadata.keywords,
    alternates: { canonical: `/iptv-vs-${competitor}` },
    openGraph: { title: data.metadata.title, description: data.metadata.description, url: `${SITE_URL}/iptv-vs-${competitor}`, type: 'article' },
  };
}
...
export default async function VersusPage({ params }: { params: Promise<{ competitor: string }> }) {
  ...
  const productSchema = {
    '@context': 'https://schema.org', '@type': 'Product', name: 'Mzansi Stream',
    description: data.content.intro, brand: { '@type': 'Brand', name: 'Mzansi Stream' },
  ...
  return (
    <main style={mainStyle}>
      ...
      <article style={articleStyle}>
        <p style={breadcrumbStyle}><Link href="/">Home</Link> / Comparison / vs {competitor}</p>
        <h1 style={h1Style}>{data.metadata.h1}</h1>
        <p style={leadStyle}>{data.content.intro}</p>
        <div style={priceBoxStyle}>💰 <strong>{data.content.pricingDelta}</strong></div>
        <a style={ctaStyle} href={wa(`Hi! I want to switch to Mzansi Stream (vs ${competitor}).`)} target="_blank" rel="noreferrer">💬 Switch to Mzansi Stream</a>
        <h2 style={h2Style}>What the competition does well</h2>
        <ul style={listStyle}>{data.content.pros.map((p) => (<li key={p}>{p}</li>))}</ul>
        <h2 style={h2Style}>The downsides</h2>
        <ul style={listStyle}>{data.content.cons.map((c) => (<li key={c}>{c}</li>))}</ul>
        ...
        <div style={ctaBlockStyle}>
          <h3 style={{ marginTop: 0 }}>Cancel and save today</h3>
          <p>No contract. 10-minute activation. WhatsApp support in English.</p>
          <a style={ctaStyle} href={wa('Hi! I want a free 24h trial of Mzansi Stream.')} target="_blank" rel="noreferrer">🧪 Free 24h trial</a>
        </div>
